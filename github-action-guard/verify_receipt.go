// Command verify_receipt gates irreversible CI steps by verifying an OER v1 receipt offline.
// See docs/integrations/github_action.md and docs/specs/VERIFIER_SDK.md.
package main
...
func run() int {
	...
	flagAction := flag.String("action-file", "", "Path to UTF-8 action JSON (or env TG_ACTION_FILE, default action.json)")
	...
	if receipt == "" {
		fmt.Fprintln(os.Stderr, "verify_receipt: missing receipt (TG_EXECUTION_RECEIPT or -receipt)")
		return 1
	}
	if surface == "" {
		fmt.Fprintln(os.Stderr, "verify_receipt: missing surface (TG_SURFACE or -surface)")
		return 1
	}
	if pubHex == "" {
		fmt.Fprintln(os.Stderr, "verify_receipt: missing public key (TG_PUBLIC_KEY_HEX or -public-key-hex)")
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "verify_receipt: read action file: %v\n", err)
		return 1
	}

	pub, err := parsePublicKeyHex(pubHex)
	if err != nil {
		fmt.Fprintf(os.Stderr, "verify_receipt: %v\n", err)
		return 1
	}
	...
	if nowStr == "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "verify_receipt: invalid now-unix: %v\n", err)
			return 1
		}
		nowUnix = n
	}
